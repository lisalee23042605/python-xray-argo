...
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'lisatunnel.qzz.io')        # Argo固定隧道域名,留空即使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiMzY2MWU0N2ZmMTU1NGFlZmIyNjI3YWIwY2NmZjZiNTkiLCJ0IjoiYTI3ZTliNjMtNzU1MC00NWVmLWJlNDAtMmFkZWE1YTQ4NDMzIiwicyI6IlptUTBNbVJqTlRJdE16TmlNeTAwTVRkaExXRmpNVEF0TTJFeU5HSmtOVGcxTkRFeiJ9')            # Argo固定隧道密钥,留空即使用临时隧道
ARGO_PORT = int(os.environ.get('ARGO_PORT', '24700'))   # Argo端口,使用固定隧道token需在cloudflare后台设置端口和这里一致
CFIP = os.environ.get('CFIP', 'cdns.doon.eu.org')       # 优选ip或优选域名
CFPORT = int(os.environ.get('CFPORT', '443'))          # 优选ip或优选域名对应端口
NAME = os.environ.get('NAME', 'Galaxy cloud')                   # 节点名称
